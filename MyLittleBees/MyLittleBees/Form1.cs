    ...
    public partial class Form1 : Form
    {
        ...
        private void SendMessage(int ID, string Message)
        {
            ...
            foreach (var group in beeGroups)
            {
                ...
                listBox1.Items.Add(group.Key.ToString() + ": "
                                 + group.Count() + " abelha" + s);
                if (group.Key == BeeState.Idle
                 && group.Count() == world.Bees.Count()
                 && framesRun > 0)
                {
                    listBox1.Items.Add("Simulação acabou : Todas as abelhas estão aposentadas");
                    toolStrip1.Items[0].Text = "Simulação acabou";
                    statusStrip1.Items[0].Text = "Simulação acabou";
                    timer1.Enabled = false;
                }
            }
        }
        ...
        private void saveToolStripButton_Click(object sender, EventArgs e)
        {
            ...
            if (saveDialog.ShowDialog() == DialogResult.OK)
            {
                ...
                {
                    MessageBox.Show("Impossível salvar a simulação no arquivo\r\n" + ex.Message,
        ...
        private void openToolStripButton_Click(object sender, EventArgs e)
        {
            ...
            openDialog.Title = "Escolha um arquivo com uma simulação válida";
            if (openDialog.ShowDialog() == DialogResult.OK)
            {
                ...
                {
                    MessageBox.Show("Impossível ler a simulação no arquivo\r\n" + ex.Message,
                    "Bee Simulator Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    world = currentWorld;
